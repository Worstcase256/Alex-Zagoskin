...
public:
	Department()
	{ 
		this->departmentName = "Traffic Management";
		this->Boss = "Nikita Kozlov";
	}
	Department(string n_department, string boss) 
	{ 
		this->departmentName = n_department; 
		this->Boss = boss; 
	}

	void setN_departHumant(string n_department) { this->departmentName = n_department; }
	void setBoss(string boss) { this->Boss = boss; }

	string getN_departHumant() { return this->departmentName; }
	string getBoss() { return this->Boss; }
	list<Employee> getEmployees() { return Employees; }

	void AddEmployees(Employee listEmployeers) { this->Employees.push_back(listEmployeers); }

	friend ostream& operator<<(ostream& ost, Department& department)
	{
		ost << "Department Name: " << department.departmentName << endl;
		ost << "Department boss: " << department.Boss<< endl;
		return ost;
	}
};
...
int main()
{
	...
	Employee* Employ1 = new Employee("Ivan", "Petrov", "Janitor", 176, 31.63);
	Employee* Employ2 = new Employee("Nikolay", "Sidorov", "Handyman", 180, 34.37);
	Employee* Employ3 = new Employee("Olga", "Nikolaevna", "Programmer", 176, 89.91);
	Employee* Employ4 = new Employee("Maksim", "Makarov", "Driver", 176, 39.17);
	Employee* Employ5 = new Employee("Tatyana", "Beseda", "Accountant", 7512.11, 22, 3000);
	Employee* Employ6 = new Employee("Inna", "Borisovna", "Financial analyst", 12512.11, 20, 5000);
	Employee* Employ7 = new Employee("Artur", "Victorovich", "Director ", 35000, 0.49);
	Employee* Employ8 = new Employee("Anna", "Alekseevna", "Sales Manager ", 17000, 0.195);
	
	Employes.push_back(Employ1);
	Employes.push_back(Employ2);
	Employes.push_back(Employ3);
	Employes.push_back(Employ4);
	Employes.push_back(Employ5);
	Employes.push_back(Employ6);
	Employes.push_back(Employ7);
	Employes.push_back(Employ8);

	vector<Employee*>::iterator ptr;
	
	for (ptr = Employes.begin(); ptr < Employes.end(); ptr++)
	{
		cout << *(*ptr) << " " << endl;
	}

	Department department("Traffic Management", "Nikita Kozlov");
	cout << department;
